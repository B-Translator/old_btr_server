...
*** List of *string-->translations*
    + [X] Define and implement suitable access rights.
    + [X] Some of the advanced filtering options should be available
      only if the user has propper access rights.
    + [X] Some editing options should be available only if the user has
      propper access rights.
    + [X] For each suggestion, the admin (or moderators) should be able to apply
    ...
    + [X] For each suggestion, display details like: suggested by, suggested
    ...
    + [X] When the votes are changed, the old (removed) votes should be stored on
...
*** TODO Create a dictionary/thesaurus with the most interesting terms and translations
