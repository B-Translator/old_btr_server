...
* Making a clone for development
  ...
  *Caution:* The root directory and the DB of the clone will be
  ...
  chroot /var/chroot/btr/
  cd /var/www/
  drush php-script btranslator/profiles/btranslator/config.php
  drush @dev php-script btranslator/profiles/btranslator/config.php
  #+END_EXAMPLE

  It will make site_name 'B-Translator (dev)', will make site email
  something like 'user+dev@gmail.com', will enable email re-routing,
  display the /devel/ menu of the footer region, etc. It is not
