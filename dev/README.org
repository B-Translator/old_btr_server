...
* Making a clone for development
  ...
  *Caution:* The root directory and the DB of the clone will be
  ...
  drush @dev php-script btr/profiles/btr_server/config.php dev2
  #+END_EXAMPLE
...
* Making a backup of the application
  ...
  btr_server/dev/snapshot.sh make @dev
  #+END_EXAMPLE
  ...
  btr_server/dev/snapshot.sh restore @dev --file=snapshot-btr-20130602.tgz
  btr_server/dev/snapshot.sh restore @dev --file=snapshot-btr_dev-20130602.tgz
  #+END_EXAMPLE
