...
Development Setup of B-Translator Server
...
It is developed as a Drupal7 profile and the [[https://github.com/B-Translator/btr_server][code]] is hosted on GitHub.
...
* Changing the webserver
  ...
  /var/www/btr/profiles/btr_server/dev/apache2.sh start
  /var/www/btr/profiles/btr_server/dev/apache2.sh stop
  #+END_EXAMPLE
  ...
  *Important:* When /apache2/ is started, the services /nginx/,
  ...
  #+BEGIN_EXAMPLE
  127.0.0.1  btr.example.org dev.btr.example.org
  #+END_EXAMPLE
...
* Re-installing the application
  ...
  #+BEGIN_EXAMPLE
  chroot /var/chroot/btr/
  /var/www/btr/profiles/btr_server/dev/reinstall.sh
  #+END_EXAMPLE
  It will rebuild the Drupal directory with *drush make* and install
  the btr_server profile with *drush site-install*, and then do all
  ...
  #+BEGIN_EXAMPLE
  chroot /var/chroot/btr/
  /var/www/btr/profiles/btr_server/dev/reinstall-db.sh
  #+END_EXAMPLE
...
* Making a clone for development
  ...
  DB named *btr_dev*. It also creates the drush alias *@btr_dev*, and
  modifies the configuration of the webserver so that the cloned
  application can be accessed at *dev.btr.example.org*.
  ...
  *Caution:* The root directory and the DB of the clone will be
  ...
  chroot /var/chroot/btr/
  cd /var/www/
  drush @btr php-script btr/profiles/btr_server/config.php dev1
  drush @btr_dev php-script btr/profiles/btr_server/config.php dev2
  #+END_EXAMPLE
...
* Making a backup of the application
  ...
  chroot /var/chroot/btr/
  cd /var/www/
  ln -s btr_dev/profiles/btr_server/ .

  btr_server/dev/snapshot.sh make
  btr_server/dev/snapshot.sh make @btr_dev
  #+END_EXAMPLE
  These will create the files ~snapshot-btr-20130602.tgz~
  and ~snapshot-btr_dev-20130602.tgz~. They can be restored
  like this:
  #+BEGIN_EXAMPLE
  btr_server/dev/snapshot.sh restore --file=snapshot-btr-20130602.tgz
  btr_server/dev/snapshot.sh restore --file=snapshot-btr_dev-20130602.tgz
  btr_server/dev/snapshot.sh restore @btr_dev --file=snapshot-btr-20130602.tgz
  btr_server/dev/snapshot.sh restore @btr_dev --file=snapshot-btr_dev-20130602.tgz
  #+END_EXAMPLE
...
* Pushing commits to github
  ...
  =/var/www/btr_dev/profiles/btr_server/= (as well as the one
  on =/var/www/btr/profiles/btr_server/=) are actually clones
  of the git repository at https://github.com/B-Translator/btr_server
...
* Making a local git clone of the dev application
  ...
  *@btr_dev* application (~/var/www/btr_dev~). For example this is the
  ...
  [[https://github.com/B-Translator/btr_server][GitHub repository]] it is easy to push commits from /btr_dev/ to
