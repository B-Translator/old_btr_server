...
function delete_template($potid)
{
  ...
  $db->exec("DELETE FROM btr_locations WHERE potid = $potid");

  // Delete the files related to this template.
  $db->exec("DELETE FROM btr_files WHERE potid = $potid");

  // Delete the template itself.
  $db->exec("DELETE FROM btr_templates WHERE potid = $potid");
}
...
function add_string($entry)
{
  ...
  $sql = "UPDATE btr_strings SET count = count + 1 WHERE sguid = '$sguid'";
  global $db;
  if (!$db->exec($sql)) {
