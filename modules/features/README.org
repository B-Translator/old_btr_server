...
* Creating features
  ...
  components/:
  #+BEGIN_EXAMPLE
  drush @btr_dev help features-components
  drush @btr_dev features-components '%:%' > features-components.txt
  #+END_EXAMPLE
  ...
  #+BEGIN_EXAMPLE
  variable:theme_btr_server_settings
  ...
  command like this:
  #+BEGIN_EXAMPLE
  drush @btr_dev features-export \
  --destination=profiles/btr_server/modules/features \
  btr_layout $(cat layout_components)
  #+END_EXAMPLE
  It creates a feature on the directory:
  ~/var/www/btr_dev/profiles/btr_server/modules/features/btr_layout/~

  Other features that I have created are: /btr_disqus/, /btr_invite/,
  /btr_captcha/, /btr_hybridauth/, /btr_simplenews/,
  /btr_mass_contact/, /btr_googleanalytics/, etc.  In order to
...
* Making features configurable
  ...
  installed when the (btr_server) profile is installed, and some
...
* Saving and restoring private variables
  ...
  works like this:
  #+BEGIN_EXAMPLE
  features/save-private-vars.sh @btr_dev
  drush @btr_dev php-script restore-private-vars.php
  #+END_EXAMPLE
